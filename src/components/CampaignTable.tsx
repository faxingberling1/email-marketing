...
export function CampaignTable({ campaigns: initialCampaigns = [], onRefresh }: CampaignTableProps) {
    ...
    const handleDelete = async (id: string) => {
        ...
        if (!result.success) {
            ...
            // Refresh sidebar badge count instantly
    ...
    const campaignToDelete = campaigns.find(c => c.id === confirmDeleteId)
            ...
            <AnimatePresence>
                {confirmDeleteId && (
                    <>
                        ...
                        >
                            <div className="rounded-3xl border border-white/10 bg-slate-900/95 p-8 shadow-2xl backdrop-blur-xl">
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <button
                                        onClick={() => setConfirmDeleteId(null)}
                                        className="h-8 w-8 flex items-center justify-center rounded-xl bg-white/5 text-slate-500 hover:text-white hover:bg-white/10 transition-all"
                                    >
                                        <X className="h-4 w-4" />
                                    </button>
                                </div>
                                <h3 className="text-lg font-black text-white mb-2">Delete Campaign?</h3>
                                <p className="text-sm text-slate-400 mb-1">
                                    <span className="text-white font-bold">{campaignToDelete?.name}</span>
                                </p>
                                <p className="text-xs text-slate-500 mb-8 leading-relaxed">
                                    This will permanently delete the campaign and all its email records. This action cannot be undone.
                                </p>
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={() => setConfirmDeleteId(null)}
                                        className="flex-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 px-4 py-3 rounded-xl bg-rose-500 hover:bg-rose-400 text-[10px] font-black text-white uppercase tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-rose-500/20"
                                    >
                                        {deletingId === confirmDeleteId ? 'Deleting...' : 'Delete Campaign'}
                                    </button>
                                </div>
                            </div>
                        </motion.div>
